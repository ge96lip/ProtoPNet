                ...
                for prototype_idx in sorted_pooled_indices:
                    ...
                    if abs(simweight) > 0.01:
                        ...
                        if use_opencv:
                            ...
                            # Ensure img_tensor has three channels by dropping the alpha channel if present
                            img_tensor = img_tensor.squeeze().numpy().transpose(1, 2, 0)
                            if img_tensor.shape[2] == 4:
                                img_tensor = img_tensor[:, :, :3]

                            # Now, both heatmap and img_tensor should have the shape (224, 224, 3)
                            heatmap_img = 0.2 * np.float32(heatmap) + 0.6 * np.float32(img_tensor)
