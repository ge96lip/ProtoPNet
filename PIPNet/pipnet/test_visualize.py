...
def vis_pred2(net, vis_test_dir, classes, device, args: argparse.Namespace):
    ...
    for k, (xs, ys) in enumerate(vis_test_loader):  # shuffle is false so should lead to same order as in imgs
        if ys[0] != last_y:
            ...
            count_per_y += 1
            if count_per_y > 100:  # show max 5 imgs per class to speed up the process
                continue
...
def vis_pred_experiments2(net, imgs_dir, classes, device, args: argparse.Namespace):
    # Make sure the model is in evaluation mode
    net.eval()
