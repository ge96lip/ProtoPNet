...
import medmnist
from src.models.XProtoNet import XProtoNet  # Import the custom model
...
class evaluation:
    def __init__(self, model_path, config, label_number, data_flag, batch_size=64, output_dir='output_images'):
    ...
    def visualize_prototypes(self, image, distances, influences, prediction, image_index):
        ...
        axs[0].set_title(f'Original Image - {self.class_names[prediction]}')
        axs[0].axis('off')

        # Determine number of prototypes per class
        num_prototypes_per_class = len(influences) // self.label_number

        # Normalize influences to the range [0, 1]
        # Create overlays for each class
        total_influence = 0

        # First loop: Calculate the total influence of the top 3 prototypes per class
        for class_id in range(self.label_number):
            class_influences = influences[class_id * num_prototypes_per_class:(class_id + 1) * num_prototypes_per_class]
            top_indices = torch.argsort(class_influences, descending=True)[:3]  # Get top 3 influences for clarity
            total_influence += sum(class_influences[id].item() for id in top_indices)

        # Second loop: Create heatmaps and overlays for each class
        for class_idx in range(self.label_number):
            class_influences = influences[class_idx * num_prototypes_per_class:(class_idx + 1) * num_prototypes_per_class]
            top_indices = torch.argsort(class_influences, descending=True)[:3]  # Get top 3 influences for clarity
            combined_heatmap = self.create_combined_heatmap(top_indices, class_influences, distances, total_influence)
            heatmap = get_heatmap(combined_heatmap[None, ...])[0]
            overlay = self.overlay_heatmap(image_np, heatmap)
            
            axs[class_idx + 1].imshow(overlay)
            axs[class_idx + 1].set_title(f'{self.class_names[class_idx]}')
    ...
    def create_combined_heatmap(self, top_indices, influences, distances, total_influence):
        ...
        for idx in top_indices:
            ...
            combined_heatmap += (influence / total_influence) * distance_map_resized
    ...
    def evaluate(self):
        with torch.no_grad():
            for images, labels in self.test_loader:
                ...
                break #just one batch you can delete break if you want

if __name__ == '__main__':
    model_path = 'logs/Image_ProtoASNet/blood/bloodmnist_10prototypes.pth'  # Update with the correct path
    # Define your configuration here
    config = {
        'features': resnet50_features(pretrained=True),
        'num_classes': 8,
        'prototype_shape': (80, 1024, 1, 1),
        'img_size': 224,
        'proto_layer_rf_info': {
    ...
    evaluator = evaluation(model_path=model_path, config=config, label_number=label_number, data_flag=data_flag)
    evaluator.evaluate()