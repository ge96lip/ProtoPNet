...
class L_norm(object):
    ...
    def compute(self, tensor, dim=None):
        # Debug statements to print tensor information
...
class CeLossAbstain(object):
    ...
    def compute(self, logits, target):
        ...
        class_pred = logits[:, :K+1].softmax(dim=1)  # (B, K)
        ...
        target_oh = nn.functional.one_hot(target, num_classes=K+1).float() # K
        
        abs_pred = abs_pred.expand(-1, K+1)

        class_pred = torch.cat([class_pred, torch.zeros(B, 1, device=logits.device)], dim=1)

        virtual_pred = (1 - abs_pred) * class_pred + abs_pred * target_oh  # (B, K+1)
        
        # Compute the loss
        loss_pred = self.criterion(torch.log(virtual_pred), target)
        loss_abs = -torch.log(1 - abs_pred[:, 0]).squeeze()  # (B
        ...
        target_oh = nn.functional.one_hot(target, num_classes= K + 1)
