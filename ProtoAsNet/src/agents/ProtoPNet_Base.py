    ...
    def train(self):
        ...
        for epoch in range(self.current_epoch, self.train_config["num_train_epochs"]):
            ...
            if epoch < self.train_config["num_warm_epochs"]:
                self.warm_only()
                acc,accu, mean_f1, auc = self.run_epoch(epoch, self.warm_optimizer, mode="train")
            else:
                self.joint()
                acc,accu, mean_f1, auc = self.run_epoch(epoch, self.joint_optimizer, mode="train")
                self.scheduler.step()
